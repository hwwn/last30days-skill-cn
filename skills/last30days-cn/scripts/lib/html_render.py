"""HTML rendering for shareable last30days-cn reports (中文双语版)."""
...
# 面向用户的散文标签（host 模型按 SKILL.md 双语合约 §7 输出）。
# 第一元素 = 模型实际写出的中文标签行（被提升为 ## 标题）；
# 第二元素 = 提升后的标题文案（双语：中文主、英文括注）。
# 同时保留英文原文作为回退，避免模型偶发输出英文标签时漏提升。
PROSE_LABELS = [
    ("我了解到：", "我了解到 (What I learned)"),
    ("研究中的关键模式：", "研究中的关键模式 (Key patterns from the research)"),
    # 英文回退（模型偶发英文输出时仍能正确提升为标题）。
    ("What I learned:", "我了解到 (What I learned)"),
    ("KEY PATTERNS from the research:", "研究中的关键模式 (Key patterns from the research)"),
]

# 邀请语整块剥离（不进入可分享 HTML）。匹配 host 模型按 SKILL.md 双语合约输出的
# 中文邀请块（「我现在是…的专家了……尽管问。」），并保留英文原文作为回退。
INVITATION_PATTERN = re.compile(
    r"^---\n(?:我现在(?:已经)?是|I'm now an expert).*?(?:尽管问。|Just ask\.)$",
...
html, body {
  ...
  background: var(--bg);
  color: var(--fg);
  font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'PingFang SC', 'Hiragino Sans GB', 'Microsoft YaHei', system-ui, sans-serif;
  font-size: 17px;
  line-height: 1.65;
...
<title>last30days-cn · __TITLE__</title>
...
def _markdown_to_html(md: str) -> str:
    ...
    while index < len(lines):
        ...
        # 徽章探测：CN 徽章首行为 `🌐 最近30天 · last30days-cn v{VERSION} · 已同步 {YYYY-MM-DD}`
        # （契约 §7）。匹配中文徽章前缀，剥离 🌐 后注入 accent span，结构与上游一致。
        if stripped.startswith("🌐 最近30天"):
            flush_paragraph()
            close_list()
...
def _build_colophon(report: schema.Report, *, topic: str | None = None) -> str:
    ...
    rerun = html.escape(f"/last30days-cn {display_topic}")
    return (
        '<div class="colophon">\n'
        f"  生成于 {generated} · /last30days-cn v{html.escape(version)} · 话题：{escaped_topic}<br>\n"
        f'  获取最新数据请重跑：<span class="rerun">{rerun}</span>\n'
        "</div>"
    )
