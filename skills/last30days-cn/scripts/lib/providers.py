...
# --- 国产大模型（OpenAI 兼容 /chat/completions 形态）---
# DeepSeek / 通义千问(DashScope) / Kimi(Moonshot) / 智谱 GLM 都暴露 OpenAI 风格
# 的 /chat/completions 端点，因此共用 OpenRouterClient 的请求/解析模式。
...
MOONSHOT_DEFAULT = "moonshot-v1-8k"
ZHIPU_URL = "https://open.bigmodel.cn/api/paas/v4/chat/completions"
ZHIPU_DEFAULT = "glm-4-flash"
...
class _OpenAICompatClient(ReasoningClient):
    """国产大模型的共享基类：OpenAI 兼容 /chat/completions。

    DeepSeek / DashScope(通义) / Moonshot(Kimi) / 智谱 GLM 的请求体与响应解析
    形态与 OpenRouter 完全一致，仅端点 URL 与默认模型不同，故抽出公共实现，
    子类只需声明 name 与 endpoint。
    """
...
class DashScopeClient(_OpenAICompatClient):
    name = "dashscope"
...
_MODEL_DEFAULTS: dict[str, tuple[str, str]] = {
    ...
    "deepseek": (DEEPSEEK_DEFAULT, DEEPSEEK_DEFAULT),
    "dashscope": (DASHSCOPE_DEFAULT, DASHSCOPE_DEFAULT),
    "moonshot": (MOONSHOT_DEFAULT, MOONSHOT_DEFAULT),
    "zhipu": (ZHIPU_DEFAULT, ZHIPU_DEFAULT),
}
...
def resolve_runtime(config: dict[str, Any], depth: str) -> tuple[schema.ProviderRuntime, ReasoningClient | None]:
    ...
    if provider_name == "auto":
        ...
        elif config.get("MOONSHOT_API_KEY"):
            provider_name = "moonshot"
        elif config.get("ZHIPU_API_KEY"):
            provider_name = "zhipu"
        else:
            return schema.ProviderRuntime(
    ...
    if provider_name == "deepseek":
        ...
        if not deepseek_key:
            raise RuntimeError("DeepSeek selected but DEEPSEEK_API_KEY is not configured.")
        runtime = schema.ProviderRuntime(
        ...
        return runtime, DeepSeekClient(deepseek_key)

    if provider_name == "dashscope":
        dashscope_key = config.get("DASHSCOPE_API_KEY")
        if not dashscope_key:
            raise RuntimeError("DashScope (Qwen) selected but DASHSCOPE_API_KEY is not configured.")
        runtime = schema.ProviderRuntime(
        ...
        return runtime, DashScopeClient(dashscope_key)

    if provider_name == "moonshot":
        moonshot_key = config.get("MOONSHOT_API_KEY")
        if not moonshot_key:
            raise RuntimeError("Moonshot (Kimi) selected but MOONSHOT_API_KEY is not configured.")
        runtime = schema.ProviderRuntime(
            reasoning_provider="moonshot",
            planner_model=planner_model,
            rerank_model=rerank_model,
            x_search_backend=_resolve_x_backend(config),
        )
        return runtime, MoonshotClient(moonshot_key)

    if provider_name == "zhipu":
        zhipu_key = config.get("ZHIPU_API_KEY")
        if not zhipu_key:
            raise RuntimeError("Zhipu (GLM) selected but ZHIPU_API_KEY is not configured.")
        runtime = schema.ProviderRuntime(
        ...
        return runtime, ZhipuClient(zhipu_key)

    raise RuntimeError(f"Unsupported reasoning provider: {provider_name}")


def _resolve_x_backend(config: dict[str, Any]) -> str | None:
    # 中国版无 X/Twitter 后端；保留字段仅为兼容 ProviderRuntime 结构，恒为 None。
    return None
