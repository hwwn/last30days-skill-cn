...
中文移植说明（last30days-cn）：
- 数据源全部替换为中国规范源名（见 PORT_CONTRACT §1）：
  weibo / zhihu / bilibili / douyin / xiaohongshu / v2ex / juejin / github / xueqiu / grounding
- _infer_intent 增加中文正则（对比/预测/how_to/factual/opinion/breaking_news）。
- _build_prompt 文案双语化（中英），示例换为中国话题。
- 其余流水线逻辑（fallback、subquery 裁剪、能力路由、intent-modifier 扇出等）
  与上游保持一致，仅替换源名与正则。
"""
...
# 源名映射（参考西方原型）：
#   x→weibo, reddit→zhihu, hackernews→v2ex/juejin, youtube→bilibili,
#   tiktok→douyin, instagram→xiaohongshu, polymarket→xueqiu, github→github,
#   grounding→grounding。xquik（X 快速通道）在中国版无对应，已去除。
QUICK_SOURCE_PRIORITY = {
    "factual": ["v2ex", "zhihu", "weibo", "juejin", "bilibili"],
    "product": ["bilibili", "zhihu", "weibo", "xiaohongshu", "douyin"],
    "concept": ["v2ex", "zhihu", "weibo", "juejin", "bilibili"],
    "opinion": ["zhihu", "weibo", "bilibili", "v2ex", "xiaohongshu"],
    "how_to": ["bilibili", "zhihu", "weibo", "juejin", "v2ex"],
    "comparison": ["zhihu", "weibo", "v2ex", "juejin", "bilibili"],
    "breaking_news": ["weibo", "zhihu", "v2ex", "bilibili", "douyin", "xueqiu"],
    "prediction": ["xueqiu", "weibo", "v2ex", "zhihu", "bilibili"],
}
SOURCE_PRIORITY = {
    "factual": ["v2ex", "zhihu", "weibo", "bilibili"],
    "product": ["bilibili", "zhihu", "weibo", "xiaohongshu", "douyin", "v2ex"],
    "concept": ["v2ex", "zhihu", "weibo", "bilibili"],
    "opinion": ["zhihu", "weibo", "bilibili", "v2ex"],
    "how_to": ["bilibili", "zhihu", "weibo", "juejin", "v2ex"],
    "comparison": ["zhihu", "weibo", "v2ex", "bilibili"],
    "breaking_news": ["weibo", "zhihu", "v2ex", "bilibili", "xueqiu"],
    "prediction": ["xueqiu", "weibo", "v2ex", "zhihu", "bilibili"],
}
SOURCE_LIMITS = {
    "quick": {
...
def plan_query(
    *,
    ...
    if context:
        prompt += f"\n\nCurrent context (from web search) / 当前上下文（来自网页搜索）: {context}"
    ...
    if not internal_subrun:
        ...
        print(
            ...
            "（未传入 --plan：若你就是承载本 skill 的推理模型，你即 planner，"
            "请自行生成 JSON 查询计划并通过 --plan 传入；无需 API key。下方确定性"
            "回退仅用于 headless/cron 路径。）",
            file=sys.stderr,
        )
...
def _build_prompt(
    ...
    return f"""
You are the query planner for a live last-30-days research pipeline (China market).
你是「最近30天」实时研究流水线（中国市场版）的查询规划器。

Topic / 话题: {topic}
Depth / 深度: {depth}
Available sources / 可用数据源: {available}
Requested sources / 指定数据源: {requested}

Return JSON only with this shape / 仅返回如下结构的 JSON:
{{
  ...
  "subqueries": [
    {{
      "label": "short label / 简短标签",
      "search_query": "keyword style query for search APIs / 关键词式检索串",
      "ranking_query": "natural language rewrite for reranking / 用于重排的自然语言问句",
      "sources": ["zhihu", "weibo", "grounding"],
      "weight": 1.0
    }}
  ],
  "notes": ["optional short notes / 可选简短备注"]
}}

Rules / 规则:
- emit 1 to 5 subqueries (how_to/opinion/product/breaking_news intents benefit from 4-5; factual/concept from 2)
  生成 1 到 5 条子查询（how_to/opinion/product/breaking_news 适合 4-5 条；factual/concept 适合 2 条）
- every subquery must include both search_query and ranking_query
  每条子查询必须同时包含 search_query 与 ranking_query
- sources must be drawn from Available sources only
  sources 只能取自上面的「可用数据源」
- use cluster_mode=none for factual or many how-to queries
  factual 或多数 how-to 查询用 cluster_mode=none
- use strict_recent for breaking news and most predictions
  突发新闻与多数预测类用 strict_recent
- use debate for comparison/opinion, market for prediction, workflow for how_to, story for breaking_news
  对比/观点用 debate，预测用 market，how_to 用 workflow，突发用 story
- search_query should be concise and keyword-heavy
  search_query 应简洁、以关键词为主
- ranking_query should read like a natural-language question
  ranking_query 应读起来像一句自然语言问句
- preserve exact proper nouns and entity strings from the topic
  保留话题中的精确专有名词与实体字符串（如「DeepSeek」「小米 SU7」「鸿蒙」）
- NEVER include temporal phrases in search_query: no 'last 30 days', 'recent', month names, year numbers
  search_query 中绝不要出现时间短语：不要「最近30天」「最新」「近期」、月份、年份数字
- NEVER include meta-research phrases: no 'news', 'updates', 'public appearances', 'latest developments'
  绝不要出现元研究短语：不要「新闻」「动态」「最新进展」「公开动态」
- INTENT-MODIFIER HANDLING: when the topic contains one of {{use cases, use case, workflows, workflow, examples, tutorial, tutorials, review, reviews, comparison, applications, in practice, production, production use, how i use, 用法, 用例, 案例, 实战, 教程, 评测, 体验, 实践, 落地, 应用场景, 怎么用}}, STRIP that phrase from every search_query (keep its meaning in ranking_query). Emit 4-5 paraphrased subqueries that each express the intent differently (e.g., 'production / 生产环境', 'workflow OR pipeline / 工作流', 'review OR experience / 评测 体验', 'vs COMPETITOR / 对比 友商', 'community discussion / 社区讨论'). Broad retrieval, narrow ranking. This was the 2026-04-19 Hermes Agent Use Cases failure mode: the planner echoed "hermes agent use cases" as a literal search string and returned near-zero results because nobody posts that exact phrase.
  意图修饰词处理：当话题包含「用法/用例/案例/实战/教程/评测/体验/实践/落地/应用场景/怎么用」等修饰词时，从每条 search_query 中剥离该短语（含义保留在 ranking_query），并扇出 4-5 条换不同说法的子查询（如「生产环境」「工作流 OR 流水线」「评测 OR 体验」「对比 友商」「社区讨论」）。宽召回、窄重排。这是 2026-04-19「Hermes Agent Use Cases」事故模式：planner 把整句原样当检索串导致几乎零结果，因为没人会原样发这句话。
- DO NOT quote the user's full topic verbatim in search_query. Quote only multi-word proper nouns like "Claude Code", "小米 SU7", "DeepSeek R1". Bare keywords OR'd together retrieve more than exact-phrase searches.
  不要把用户整句话题加引号原样检索。只给多词专有名词加引号（如「Claude Code」「小米 SU7」「DeepSeek R1」）。裸关键词 OR 组合比精确短语召回更多。
- search_query should match how content is TITLED on platforms
  search_query 应贴近内容在平台上的标题写法
- GitHub (Issues/PRs) is best for engineering, developer tools, and open source topics: 'deepseek 推理' not 'deepseek 模型新闻 2026年3月'
  GitHub（Issues/PR）最适合工程、开发者工具与开源话题：用「deepseek 推理」而非「deepseek 模型新闻 2026年3月」
""".strip()
...
def _fallback_plan(
    ...
    if depth != "quick" and intent == "comparison":
            ...
            for index, entity in enumerate(entities, start=1):
                subqueries.append(
                    schema.SubQuery(
                        ...
                        search_query=entity,
                        ranking_query=f"在对比「{topic}」中，最近30天里与 {entity} 最相关的证据有哪些？ / What recent evidence from the last 30 days is most relevant to {entity} in the comparison '{topic}'?",
                        sources=list(source_weights),
                        weight=0.65,
                    )
                )
        ...
        subqueries.append(
            schema.SubQuery(
                label="odds",
                search_query=f"{base_search} 赔率 预期 走势",
                ranking_query=f"关于「{topic}」当前的赔率、预测或市场信号是怎样的？ / What are the current odds, forecasts, or market signals about {topic}?",
                sources=[source for source in source_weights if source in {"xueqiu", "grounding", "weibo", "zhihu"}] or list(source_weights),
                weight=0.7,
            )
        )
        ...
        subqueries.append(
            schema.SubQuery(
                label="reaction",
                search_query=f"{base_search} 反应 进展",
                ranking_query=f"关于「{topic}」最近30天有哪些新的反应或后续报道值得关注？ / What new reactions or follow-up reporting from the last 30 days matter for {topic}?",
                sources=[source for source in source_weights if source in {"weibo", "zhihu", "grounding", "v2ex"}] or list(source_weights),
                weight=0.7,
            )
        )
...
def _infer_intent(topic: str) -> str:
    ...
    if re.search(r"\b(vs|versus|compare|compared to|difference between)\b", text):
        return "comparison"
    if re.search(r"对比|对决|哪个好|哪个更|谁更|区别|相比", topic):
        return "comparison"
    ...
    if re.search(r"\b(tournament|championship|playoffs|march madness|world cup|olympics|super bowl|final four|ceremony|awards|keynote)\b", text):
        return "breaking_news"
    if re.search(r"决赛|总决赛|冠军|世界杯|奥运|颁奖|发布会|开幕|揭幕", topic):
        return "breaking_news"
    ...
    if re.search(r"\b(trending|this week|right now|today|this month)\b", text):
        return "breaking_news"
    if re.search(r"近期|本周|这个月|当下|现在|最近", topic):
        return "breaking_news"
...
def _default_source_weights(intent: str, sources: list[str]) -> dict[str, float]:
    ...
    if intent == "prediction":
        for source, bonus in {"xueqiu": 2.5, "weibo": 1.3}.items():
            if source in base:
                base[source] += bonus
    elif intent == "breaking_news":
        for source, bonus in {"weibo": 1.5, "zhihu": 1.3, "v2ex": 0.8}.items():
            if source in base:
                base[source] += bonus
    elif intent == "how_to":
        for source, bonus in {"bilibili": 2.0, "v2ex": 0.8}.items():
            if source in base:
                base[source] += bonus
    elif intent == "factual":
        for source, bonus in {"zhihu": 0.8, "weibo": 0.5}.items():
            if source in base:
                base[source] += bonus
    return base
...
def _keyword_query(topic: str, core: str) -> str:
    ...
    中文话题无空格分词，CJK 串通常本身已是紧凑短语，故不另加引号；
    仅对话题中出现的拉丁文多词专有名词加引号（如「Claude Code」）。
    """
...
def _ranking_query(topic: str, core: str) -> str:
    if topic.strip().endswith("?") or topic.strip().endswith("？"):
        return topic.strip()
    if core and core.lower() != topic.lower():
        return f"最近30天里，与「{topic}」（尤其是「{core}」）最相关的证据有哪些？ / What recent evidence from the last 30 days is most relevant to {topic}, especially about {core}?"
    return f"最近30天里，与「{topic}」最相关的证据有哪些？ / What recent evidence from the last 30 days is most relevant to {topic}?"


# Trailing-context stripping for Latin-script comparison parts. CJK 话题不依赖
# 这些英文虚词，但保留以处理混排（如 "小米 SU7 vs 问界 M9 in 续航"）的英文段。
...
def _comparison_entities(topic: str) -> list[str]:
    ...
    # 中文对比连接词归一化：「A 对比 B」「A 和 B 哪个好」「A 与 B 的区别」→ 统一切分点
    normalized = re.sub(r"对比|对决|相比|的区别|区别|哪个好|哪个更好|哪个更", " vs ", normalized)
    parts = [
        part.strip(" \t\r\n?.,:;!()[]{}\"'？。，、：；！（）「」『』")
        for part in re.split(r"\bvs\.?\b|\bversus\b|/|、|与|和", normalized, flags=re.I)
        if part.strip(" \t\r\n?.,:;!()[]{}\"'？。，、：；！（）「」『』")
    ]
...
def _has_intent_modifier(topic: str) -> bool:
    ...
    中文事故等价：原样检索「XX 用例 / XX 实战」几乎零结果，应剥离修饰词后扇出。
    """
    text = topic.lower()
...
def _intent_modifier_subqueries(
    ...
    中文版同样剥离修饰词，扇出 3 条换说法的子查询（工作流/生产落地/上手体验）。
    """
    ...
    return [
        schema.SubQuery(
            label="workflows",
            search_query=f"{entity} 工作流 流水线",
            ranking_query=f"大家用 {entity} 跑出了哪些真实的工作流或流水线？ / What real-world workflows or pipelines are people running with {entity}?",
            sources=sources,
            weight=0.6,
        ),
        schema.SubQuery(
            label="production",
            search_query=f"{entity} 生产 落地 实战",
            ranking_query=f"有哪些 {entity} 的生产部署或真实落地用例正在被讨论？ / What production deployments or real-world use cases of {entity} are people describing?",
            sources=sources,
            weight=0.55,
        ),
        schema.SubQuery(
            label="experience",
            search_query=f"{entity} 体验 评测",
            ranking_query=f"最近30天有哪些关于 {entity} 的上手体验或评测？ / What hands-on experience reports or reviews of {entity} exist in the last 30 days?",
            sources=sources,
            weight=0.5,
        ),
    ]
